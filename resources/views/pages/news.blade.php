                ...
                @if($items)
                    <div class="columns-1 md:columns-2 lg:columns-3">
                        @foreach($items  as $item)
                            <div class="relative article-item w-full mb-3 mt-10 transition-all bg-gray-100 hover:bg-gray-200 shadow rounded-md duration-300 ease-in-out hover:scale-105">
                                <div class="head p-4 border-gray-200">
                                    <div class="w-full h-64 mb-3">
                                        @if($item['img'])
                                            <img src="{{$item['img']}}" alt="{{$item['title']}}" class="object-cover h-64 bg-black mx-auto transition">
                                        @else 
                                            <img src="{{ asset('storage/tbm.jpg') }}" alt="{{$item['title']}}" class="object-cover h-64 bg-black mx-auto transition">
                                        @endif
